...
def fetch_from_peers(peer_addrs: Iterable[str], resource: str, is_json: bool = False) -> dict:
    ...
    for peer_addr in peer_addrs:
        url = f"https://{peer_addr}:4141/{resource}"
        ...
        for retry in range(PEER_RETRIES):
            try:
                ...
                if isinstance(e, SSLError) or (isinstance(e, HTTPError) and e.response is not None and e.response.status_code > 500) or is_conn_refused(e):
                    log(__file__, f"{err_class} fetching {url} - {e}")
                    break
                else:
...
def discover_peers(candidates: Set[str]) -> Set[str]:
    attempted_peers, confirmed_peers = set(), set()
    while candidates:
        results = fetch_from_peers(candidates - localhosts, "peers.lst", is_json=False)
        confirmed_peers |= results.keys()
        attempted_peers |= candidates
        candidates = {peer for peer_list in results.values() for peer in peer_list} - attempted_peers

    new_peers = confirmed_peers - peers
    if new_peers:
        log(__file__, f"added peers: {', '.join(new_peers)} ({metrics['peers']} active peers)")
    return new_peers
...
def is_conn_refused(e: Exception) -> bool:
    try:
        return isinstance(e, ConnectionError) and isinstance(e.__context__.__context__.__context__, ConnectionRefusedError)
